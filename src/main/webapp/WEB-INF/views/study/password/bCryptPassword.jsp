...
<div class="container">
  ...
  <pre>
  	- 스프링 시큐리티(Spring Security) 프레임워크에서 제공하는 클래스중 하나로 비밀번호를 암호화 하는데 사용한다.
  	  이것은 주료 자바 서버프로그램 개발을 위해 필요한 '인증'/'권한부여' 및 '보안기능'을 제공해주는 프레임워크에 속한다.
  	- BCryptPasswordEncoder는 BCrypt 해싱함수를 사용하여 비밀번호를 인코딩해주는 메서드와 사용자에 의해 
  		제출된 비밀번호를저장소에 지정된 비밀번호와의 일치여부를 확인해주는 메소드로 제공된다.
    ...
    - 사용되는 메소드 ? 
    	...
    	2) machers(java.lang.CharSequence)
    		 제출된 인코딩 되지 않는 패스워드의 일치여부를 판단하기위해 인코딩된 패스워드와 비교 판단한다.
    		 반환타입은 boolean이다.
  </pre>
  <hr/>
  <p>
